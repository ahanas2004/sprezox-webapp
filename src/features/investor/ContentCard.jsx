...
import { PlayCircle, Mic, Heart, Bookmark, Share2 } from 'lucide-react';
import styles from './ContentCard.module.css';

export default function ContentCard({ content }) {
    // Determine if the content is a reel or podcast to show the correct icon
    const isReel = content.content_type === 'reel';
    
    // Safely access creator data from the joined 'profiles' table
    ...
    return (
        ...
        >
            <div className={styles.mediaPlaceholder}>
                {isReel ? (
                    <PlayCircle size={64} className={styles.mediaIcon} />
                ) : (
                    <Mic size={64} className={styles.mediaIcon} />
                )}
                <div className={styles.glow}></div>
                 {content.thumbnail_path && <img src={content.thumbnail_path} alt={content.title} className={styles.thumbnail} />}
            </div>
